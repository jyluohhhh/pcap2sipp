...
void packet_handler_ips( u_char *param, const struct pcap_pkthdr *header, const u_char *pkt_data )
{
    ...
    else
    {
        ...
        sc = (char *)((u_char *)th + 20 );
        sc[header->len - ip_len - 20 - 16] = '\0';
...
int handle_receive( char *text, int type )
{
    if ( type == REPLY )
    {
        ...
        if ( strncmp( text, last_reply, 3 ) != 0 )
        {
            ...
            {
                if ( strcmp( text, "200" ) == 0 )
                {
                    fprintf( scenario_file, "<recv response=\"%s\" auth=\"true\" %s><action>\n"
                     "  <ereg regexp=\"tag=([[:alnum:]-]*)\" search_in=\"hdr\" header=\"To:\" assign_to=\"3,4\" />\n"
                ...
                else if ( strcmp( text, "180" ) == 0 )
                {
                    fprintf( scenario_file, "<recv response=\"%s\" auth=\"true\" %s><action>\n"
                     "  <ereg regexp=\"branch=([[:alnum:]-]*)\" search_in=\"hdr\" header=\"Via:\" assign_to=\"1\" />\n"
                     "  <ereg regexp=\"tag=([[:alnum:]-]*)\" search_in=\"hdr\" header=\"To:\" assign_to=\"3,4\" />\n"
                     /*"  <ereg regexp=\".*\" search_in=\"hdr\" header=\"From:\" assign_to=\"5\" />\n"
                     "  <ereg regexp=\".*\" search_in=\"hdr\" header=\"To:\" assign_to=\"6\" />\n"
                     "  <ereg regexp=\"<(.*)>\" search_in=\"hdr\" header=\"Contact:\" assign_to=\"7,8\" />\n"*/
                     "</action></recv>\n", text, optional_tag );
                    fprintf( scenario_file, "<Reference variables=\"1,3,4\" />\n" );
                    fprintf( scenario_file, "\n" );
                }
                else
                {
    ...
    else
    {
        if ( (strcmp( text, "ACK" ) != 0) && (strcmp( text, "PRACK" ) != 0) )
        {
            fprintf( scenario_file, "<recv request=\"%s\"><action>\n"
                     ...
                     "</action></recv>\n", text );
            fprintf( scenario_file, "\n" );
            fprintf( scenario_file, "<Reference variables=\"1,2,3,4,5\" />\n" );
...
char* handle_to_header( char *rep, int type, char *saddr, char *daddr )
{
    ...
    if ( type == REPLY )
    {
        if ( this_is_tag_add )
        {
            return strdup( "[last_To:];tag=[call_number]" );
        }
        else
        {
    ...
    else
    {
        ...
        else if ( this_is_tag_add )
        {
            if ( newto != NULL )
            {
                free( newto );
            }
            newto = malloc( sizeof(char) * (strlen( ";tag=[$4]" ) + 1) );
            sprintf( newto, "%s%s", rep, ";tag=[$4]" );
            rep = newto;
        }
    }
...
char* handle_callid_header( char *rep, const char *saddr )
{
    size_t size;
    char *newstring;
    ...
    if ( i_am_initiator == 1 )
    {
        size = sizeof(char) * (1 + strlen( "Call-ID: [call_id]" ));
        ...
        strcpy( newstring, "Call-ID: [call_id]" );
        rep = newstring;

        /*rep = replace( rep, "Call-ID", "Call-ID: [call_id]", IP_STRING );
        if ( rep == NULL )
            return NULL;*/
    }
    else
...
char* handle_cseq_header( char *rep, int type )
{
    ...
    if ( type == REPLY )
    {
        size = sizeof(char) * (1 + strlen( "CSeq: [cseq] INVITE" ));
        ...
        if ( this_is_487_method )
        {
            strcpy( newstring, "CSeq: [cseq] INVITE" );
            this_is_487_method = 0;
        }
        else
        {
    ...
    {
        size = sizeof(char) * (1 + strlen( "CSeq: [cseq] BYE" ));

        newstring = malloc( size );
        if ( this_is_request_bye )
        {
            strcpy( newstring, "CSeq: [cseq] BYE" );
            this_is_request_bye = 0;
        rep = newstring;
    }
    }
    return rep;
...
char* handle_via_header( char *rep, int type, char *saddr, char *daddr )
{
    ...
    else
    {
        ...
        if ( rep == NULL )
            return NULL;
        rep = replace_after( rep, "as.tekvizion.com", ";", "[local_ip]:[local_port]", 1 );
        ...
        {
            rep = replace_after( rep, ";branch=", ";", ";[$1]", 1 );
            this_is_cancel = 0;
        }
    }
    ...
    if ( (sport == 5060) || (dport == 5060) || (is_sip( sc )) )
    {
        ...
        if ( callid != NULL )
        {
            ...
            if ( strstr( sim_callid, callid ) != NULL )
            {
                ...
                if ( strstr( sc, "487" ) )
                {
                    this_is_487_method = 1;
                }
                if ( strstr( sc, "Ringing" ) )
                {
                    this_is_tag_add = 1;
                }
                if ( strstr( sc, "BYE " ) )
                {
                    this_is_request_bye = 1;
                }
                if ( strstr( sc, "CANCEL " ) )
                {
                    this_is_cancel = 1;
                ...
                if ( strcmp( sim_ip, daddr ) == 0 && dport == port )
                {
                    ...
                    while ( pch != NULL )
                    {
                        ...
                        {
                            //printf( "sa %s , da %s\n", saddr, daddr );
                            rep = handle_callid_header( rep, saddr );
                            if ( rep == NULL )
                                return;
                        }
